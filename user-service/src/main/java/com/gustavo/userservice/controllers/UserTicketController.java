...
public class UserTicketController {
	...
	public ResponseEntity<PageImplResponseDTO<TicketResponseDTO>> findAllTicketsByUser(
			@PathVariable UUID userId,
			@PageableDefault(page = 0, size = 10, sort = "ticketId", direction = Sort.Direction.ASC) Pageable pageable) {
		
		PageImplResponseDTO<TicketResponseDTO> ticketResponseDtoPage = userTicketService.findAllTicketsByUser(userId, pageable);
